...
public class HotItems {
    public static void main(String[] args) throws Exception {
        ...
        // 创建了一个 UserBehavior 类型的 DataStream
        DataStream<UserBehavior> dataSource = env.createInput(csvInput, pojoType);
        // 按照 EventTime 模式进行处理，Flink 默认使用 ProcessingTime 处理，所以我们要显式设置下
        env.setStreamTimeCharacteristic(TimeCharacteristic.EventTime);

        // 得到一个带有时间标记的数据流了，后面就能做一些窗口的操作
        ...
        // 原始数据中存在点击、加购、购买、收藏各种行为的数据，但是我们只需要统计点击量，
        // 所以先使用 FilterFunction 将点击行为数据过滤出来
